...
class UserTaskModel:
    ...
    def get_instance(cls):
        if cls._instance is None:
            cls._instance = UserTaskModel()
        return cls._instance

    def save_user_task(self, user_id, assignment_id, status="pending"):
        conn = None
        try:
            conn = get_conn()
    ...
    def update_task_status(self, user_id, assignment_id, status):
        conn = None
        try:
            conn = get_conn()
            conn.row_factory = sqlite3.Row
    ...
    def get_user_tasks_by_user_id(self, user_id):
        conn = None
        try:
            conn = get_conn()
            conn.row_factory = sqlite3.Row
    ...
    def get_user_task_by_user_id_and_assignment_id(self, user_id, assignment_id):
        conn = None
        try:
            conn = get_conn()
            conn.row_factory = sqlite3.Row
    ...
    def get_all_user_task_and_assignment_info_by_user_id(self, user_id):
        conn = get_conn()
        cursor = conn.cursor()
        cursor.execute("""
            SELECT 
